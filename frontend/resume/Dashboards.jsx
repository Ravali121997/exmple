...
useEffect(() => {
  if (!resumeId) {
    console.warn("🚫 Cannot dispatch createDashboards — resumeId not ready");
    return;
  }

  const handleCreate = async (e) => {
    const { scale } = e.detail;
    console.log("📥 createDashboards event received:", scale);
    console.log("🧩 Current resumeId before generating dashboard:", resumeId);
    setEventScale(scale);
  ...
  const handlePrompt = async (e) => {
    ...
    console.log("📥 handlePrompt CALLED", e.detail);

    setEventPrompt(prompt);

    if (!resumeId) {
      console.warn("❗Resume not uploaded yet — skipping prompt generation");
      return;
    }
  ...
  console.log("✅ resumeId is ready — adding event listeners");
  window.addEventListener("createDashboards", handleCreate);
  window.addEventListener("promptSubmit", handlePrompt);

  return () => {
    console.log("🧹 Cleaning up RightSidebar event listeners");
    window.removeEventListener("createDashboards", handleCreate);
    window.removeEventListener("promptSubmit", handlePrompt);
  };
}, [resumeId, selectedDashboardId]);
...
useEffect(() => {
  if (analysisData && !hasInteractedRef.current) {
    ...
    const extractedResumeId =
      analysisData.resume_id || analysisData.file?.resume_id || analysisData.file?.id || null;

    console.log("🧩 Extracted resumeId:", extractedResumeId);
...
// --- Fetch dashboards using resumeId ---
const handleCreateDashboards = async (count = 1) => {
  if (!resumeId) {
    setError('No resume_id found. Upload the resume first.');
    return;
  }
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/generate/${resumeId}?count=${count}`, {
      method: 'POST',
    });
...
// --- Create dashboard from custom prompt ---
const handleRunPrompt = async (promptText) => {
  if (!resumeId || !promptText) return;

  setIsLoading(true);
  setError("");

  try {
    const res = await fetch(`${API_BASE_URL}/dashboard_custom_prompt/${resumeId}`, {
      method: "POST",
...
const handleModifyDashboard = async (modificationPrompt, dashboardId = selectedDashboardId) => {
    ...
    try {
      ...
      const url = new URL(`${API_BASE_URL}/dashboard_modify/${dashboardId}`);
        ...
        <div className={`bg-white shadow-xl transition-all duration-300 overflow-y-auto border-l`}> {/* Removed isHovered for simplicity */}      
          ...
          {(isInitialView) && (
            <div className="bg-white rounded-lg p-6 shadow-md">
              <div className="mb-4">
                <h3 className="text-lg font-medium text-gray-800 mb-2">Raw resume Content</h3>
                <p className="text-sm text-gray-600 mb-4">This is the original resume content. Click "Create Dashboard" in the sidebar to generate an AI-processed analysis.</p>
