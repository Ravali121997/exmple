...
export default function Sidebar({
  ...
  const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "http://127.0.0.1:8000";
   ...
   const cacheKey = `samplePrompts_${resumeData?.resumeId}`;
   
   useEffect(() => {
        console.log("🟢 jdId from analysisData in Sidebar:", analysisData?.resumeId);
    }, [analysisData]);
    useEffect(() => {
  ...
  console.log("⛔ resumeId not available yet");
  return;
}
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE_URL}/sample_prompts/${resumeData?.resumeId}`);
...
//       window.dispatchEvent(new CustomEvent("createDashboards", { detail: { scale } }));
...
const handleCreateDashboard = (e) => {
  ...
  setTimeout(() => {
    window.dispatchEvent(
      new CustomEvent("createDashboards", { detail: { scale } })
    );
